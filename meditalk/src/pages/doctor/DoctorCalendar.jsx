...
export default function DoctorCalendar() {
  ...
  const { data: appointments, loading } = useFetch(() => getAppointments({ doctorId }), [doctorId]);
  
  const [view, setView] = useState("Month");
  const [year, setYear] = useState(new Date().getFullYear());
  const [month, setMonth] = useState(new Date().getMonth());
  const [modal, setModal] = useState(false);
  const [entry, setEntry] = useState({ type: "available", date: new Date().toISOString().slice(0, 10), note: "" });
  ...
  function handleSaveEntry() {
    toast.success(`${entry.type === "available" ? "Availability" : "Blocked time"} added.`);
    setModal(false);
  }
