...
export default function DoctorPrescriptions() {
  ...
  const [form, setForm] = useState({ patientId: "", medications: [{ ...EMPTY_MED }], additionalInstructions: "" });
  ...
  function openNew() {
    ...
    setForm({ patientId: firstPatient?.id || "", medications: [{ ...EMPTY_MED }], additionalInstructions: "" });
    setCreating(true);
  }
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleSave} loading={saving}>Save Prescription</Button>
          </>
        }
      >
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => toast.info("PDF generation — coming soon.")}><FileDown className="h-4 w-4" /> Generate PDF</Button>
            <Button variant="outline" size="sm" onClick={() => toast.info("Prescription will be visible to the patient after saving.")}><Send className="h-4 w-4" /> Send to Patient</Button>
          </div>
