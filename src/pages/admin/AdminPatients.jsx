...
import { Users, Plus, Pencil, Eye, Power, Search } from "lucide-react";
...
export default function AdminPatients() {
  ...
  function save() {
    ...
    if (editing.id) {
      setList((l) => l.map((p) => (p.id === editing.id ? { ...p, ...form } : p)));
      toast.success("Patient updated (mock)");
    } else {
      const id = "P-" + (1000 + l.length + 1);
      setList((l) => [{ id, ...form, registeredAt: new Date().toISOString().slice(0, 10) }, ...l]);
      toast.success("Patient added (mock)");
    }
    setEditing(null);
  }
  function confirmToggle() {
    setList((l) => l.map((p) => (p.id === toggle.id ? { ...p, status: toggle.next } : p)));
    toast.success(`Patient ${toggle.next === "active" ? "activated" : "deactivated"} (mock)`);
    setToggle(null);
  }
