...
export default function DoctorConsultation() {
  ...
  function handleSave() {
    toast.success("Consultation saved (mock)");
  }
  function handleComplete() {
    toast.success("Consultation completed (mock)");
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-ink/50">{patient.gender} · {patient.bloodGroup} · {patient.allergies.join(", ") || "No allergies"}</p>
          </div>
        </div>
      </Card>
      ...
      <div className="flex flex-wrap gap-2">
        <Button onClick={handleSave}><Save className="h-4 w-4" /> Save Consultation</Button>
        <Button variant="outline" onClick={() => toast.info("Prescription form (mock)")}><Pill className="h-4 w-4" /> Generate Prescription</Button>
        <Button variant="success" onClick={handleComplete}><CheckCircle2 className="h-4 w-4" /> Complete Consultation</Button>
      </div>
    </div>
  );
}
